...
$THIS_DIR/on_classpath.sh 2>&1 | FileCheck -DLLVM_FIRTOOL_VERSION="$LLVM_FIRTOOL_VERSION" $THIS_DIR/on_classpath.sh

$THIS_DIR/fetched.sh 2>&1 | FileCheck -DLLVM_FIRTOOL_VERSION="$LLVM_FIRTOOL_VERSION" $THIS_DIR/fetched.sh

$THIS_DIR/firtool_path.sh 2>&1 | FileCheck -DLLVM_FIRTOOL_VERSION="$LLVM_FIRTOOL_VERSION" $THIS_DIR/firtool_path.sh
...
rm -rf $cache
FIRTOOL_CACHE=$cache $THIS_DIR/firtool_cache.sh 2>&1 | FileCheck -DLLVM_FIRTOOL_VERSION="$LLVM_FIRTOOL_VERSION" -DFIRTOOL_CACHE="$filecheck_cache" $THIS_DIR/firtool_cache.sh
